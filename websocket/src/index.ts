...
interface WSMessagePlaceUpdate extends WSMessage {
    type: "place.update";
    data: {
        x: number;
        y: number;
...
app.ws('/place', {
    open: (ws) => {
        ...
        log.info(`[/place] client connected (${decoder.decode(ws.getRemoteAddressAsText())})`);
        ws.subscribe(SUBSCRIPTIONS_ENUM.PLACE);
    },
    close: (ws, code, message) => {
        log.info(`[/place] client disconnected (${decoder.decode(ws.getRemoteAddressAsText())})`);
    },
    message: (ws, message, isBinary) => {
        log.info(`[/place] client message (${decoder.decode(ws.getRemoteAddressAsText())})`);
